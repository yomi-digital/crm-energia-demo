...
trait PaperworkSeeder
{
    private function paperworks($mysqli)
    {
        ...
        // Conta il totale per logging e progresso
        $countResult = $mysqli->query("SELECT COUNT(*) as total FROM datatype_pratiches WHERE id > 60000");
        $totPaperworks = $countResult->fetch_assoc()['total'] ?? 0;
        dump('Total paperworks: ' . $totPaperworks);

        // Processa le pratiche in batch per evitare memory exhaustion
        $batchSize = 500;
        $offset = 0;
        $countI = 0;

        do {
            $result = $mysqli->query("SELECT * FROM datatype_pratiches WHERE id > 60000 LIMIT {$batchSize} OFFSET {$offset}");
